...
def graph_to_content(graph):
    """
    Transforme le graphe extrait par classify_and_extract_visual
    ({"flowchart_nodes": [...], "flowchart_edges": [...]}) en un texte
    narratif pensé pour la RAG :

    - Ce texte sera stocké tel quel dans l'index (content / search_text).
    - Après une recherche vectorielle, un LLM ne verra QUE ce texte, jamais
      l'image d'origine. Il faut donc que le texte porte lui-même la LOGIQUE
      du parcours (séquence, branches "si oui / si non", boucles), pas
      seulement une liste de formes -> sinon le LLM en aval ne pourra que
      décrire des bouts isolés sans comprendre l'enchaînement.
    - On termine par une liste à plat des libellés de chaque étape, pour
      maximiser le rappel du moteur de recherche sur les mots-clés métier
      (le corps narratif sert à la compréhension, la liste sert au matching).
    """
    from collections import defaultdict

    nodes = graph.get("flowchart_nodes") or []
    edges = graph.get("flowchart_edges") or []
    ...
    def describe(n):
        label = shape_labels.get(n.get("shape"), "Étape")
        txt = (n.get("text") or "").strip()
        return f"{label} : {txt}" if txt else label

    # Points de départ : nœuds "start" en priorité, sinon nœuds sans arête
    # entrante, sinon le premier nœud (filet de sécurité si l'extraction
    # est imparfaite).
    start_nodes = [n for n in nodes if n.get("shape") == "start"]
    if not start_nodes:
        start_nodes = [n for n in nodes if incoming_count.get(str(n.get("id")), 0) == 0]
    if not start_nodes:
        start_nodes = [nodes[0]]
    ...
    def walk(node_id, indent=0, path_seen=frozenset(), depth=0):
        if depth > 80:
            lines.append("  " * indent + "[chemin trop long, description tronquée]")
            return
        ...
        if node_id in path_seen:
            lines.append("  " * indent + f"→ retour à l'étape « {describe(node)} » (boucle)")
            return

        visited_global.add(node_id)
        lines.append("  " * indent + f"- {describe(node)}")

        outs = outgoing.get(node_id, [])
        next_seen = path_seen | {node_id}

        if not outs:
            return

        if len(outs) == 1:
            # Séquence simple : pas de sous-indentation, on garde un fil
            # de lecture linéaire.
            walk(str(outs[0].get("to")), indent, next_seen, depth + 1)
        else:
            # Point de décision : chaque branche est explicitée avec son
            # libellé (oui/non/etc.) et indentée, pour que le LLM en aval
            # comprenne clairement les conditions.
            for e in outs:
                branch_label = (e.get("label") or "").strip() or "(branche non nommée)"
                lines.append("  " * (indent + 1) + f"Si {branch_label} :")
                walk(str(e.get("to")), indent + 2, next_seen, depth + 1)

    for s in start_nodes:
        walk(str(s.get("id")))

    # Nœuds jamais atteints depuis un point de départ (graphe mal formé /
    # extraction imparfaite) : on les liste quand même, pour ne perdre
    # aucune information plutôt que de les faire disparaître silencieusement.
    orphans = [n for n in nodes if str(n.get("id")) not in visited_global]
    for n in orphans:
        lines.append(f"- (élément isolé du flux principal) {describe(n)}")

    decision_count = sum(1 for n in nodes if n.get("shape") == "decision")

    header = (
        f"Logigramme de procédure : {len(nodes)} étape(s), "
        f"dont {decision_count} point(s) de décision.\n\n"
        f"Déroulement du logigramme :\n"
    )

    body = "\n".join(lines)

    # Liste à plat des libellés, pour le rappel du moteur de recherche
    # (mots-clés métier qui pourraient ne pas apparaître ailleurs dans le
    # texte narratif ci-dessus).
    flat_terms = ", ".join(
        (n.get("text") or "").strip() for n in nodes if (n.get("text") or "").strip()
    )
    footer = f"\n\nÉléments du logigramme : {flat_terms}." if flat_terms else ""
...
# ==============================================================================
# 3) INGEST_PDF — fonctions externes supposées déjà définies ailleurs dans
#    votre module : fitz, pdfplumber, generate_caption_for_page,
#    generate_caption_for_image, embed_query, upload_to_azure_search,
#    upload_image_to_blob, ocr_full_document, chunk_text, upscale_if_needed.
#    process_flowchart n'est plus utilisée : get_visual_analysis() extrait
#    déjà le graphe (nœuds/arêtes) en même temps que la classification.
# ==============================================================================
        ...
        for page_num, page in enumerate(pdf.pages):
            ...
            try:
                ...
                if page_is_diagram:
                    ...
                    # page_analysis contient déjà les nœuds/arêtes extraits
                    # par le modèle de vision (get_visual_analysis) -> pas
                    # besoin d'un second appel process_flowchart sur la même
                    # image, on transforme directement ce graphe en texte.
                    caption = graph_to_content(page_analysis)
            ...
            for img_index, img in enumerate(image_list):
                try:
                    ...
                    if image_is_diagram:
                        # Idem : img_analysis porte déjà le graphe extrait,
                        # inutile de rappeler process_flowchart.
                        caption = graph_to_content(img_analysis)
                        if not caption or len(caption.split()) < 5:
                            # même filet de sécurité que pour la page entière
                            print(
                                f"⚠️ Image {img_index} page {page_num + 1} détectée "
